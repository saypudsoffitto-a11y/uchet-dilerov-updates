(()=>{
  ...
  function m(n){try{return typeof money==='function'?money(n):((+n||0).toLocaleString('ru-RU')+' ₽')}catch(_){return (+n||0).toLocaleString('ru-RU')+' ₽'}}
  ...
  function disableAutoPost(){
    try{
      if(typeof state!=='undefined'){
        ...
        const key=typeof KEY!=='undefined'?KEY:'uchet_dilerov_v8';
        localStorage.setItem(key,JSON.stringify(state));
        const cb=document.getElementById('nmAutoPost');if(cb)cb.checked=false;
      }
    }catch(e){console.error('8.9.23 disable auto post',e)}
  }
  ...
  function orderInfo(data){
    ...
    let found={};try{if(typeof nmFindDealer==='function')found=nmFindDealer(data)||{}}catch(_){ }
    ...
    let width=0;try{if(typeof nmFindRollWidth==='function')width=+nmFindRollWidth(data)||0}catch(_){ }
    if(!width){
    ...
    let items=[];try{if(typeof nmBuildItems==='function')items=nmBuildItems(data)||[]}catch(e){console.error('8.9.23 nmBuildItems',e)}
  ...
  function ensureReviewModal(){
    ...
    o=document.createElement('div');o.id='nmReviewModal8923';o.className='hidden';o.innerHTML='<div class="nmReviewBox"><div class="nmReviewHead"><h2>Проверка выгрузки NewMatRos</h2><button id="nmReviewX8923" class="secondary" type="button">Закрыть</button></div><div id="nmReviewBody8923"></div></div>';document.body.appendChild(o);
    const close=()=>{o.classList.add('hidden');showPendingBanner()};
    o.querySelector('#nmReviewX8923').onclick=close;o.addEventListener('click',e=>{if(e.target===o)close()});
    return o;
  }
  ...
  function showReview(payload){
    try{
      ...
      // This sets the app's own nmCurrent/nmPreviewItems, but auto-post is disabled.
      if(typeof showNewMatRosPreview==='function')showNewMatRosPreview(data,'проверка перед продажей');
      ...
      let warning='';
      if(!info.dealer)warning+='<div class="nmReviewWarn"><b>Дилер не найден точно.</b> Проверь имя и телефон перед оформлением. При подтверждении программа может создать карточку из NewMatRos.</div>';
      if(!info.mat||(+info.mat.price||0)<=0)warning+='<div class="nmReviewWarn"><b>Цена плёнки не определена.</b> Продажу оформлять нельзя, пока цена не будет найдена в карточке товара.</div>';
      ...
      const sale=body.querySelector('#nmReviewSale8923');
      sale.onclick=()=>{
        disableAutoPost();
        if(typeof showNewMatRosPreview==='function')showNewMatRosPreview(data,'пересчёт');
      ...
      console.error('8.9.23 NewMatRos review',e);
  ...
  function ensureBanner(){
    ...
    // Replace old 8.9.17 button to remove its handler that only switched sections.
    const old=b.querySelector('#nmLiveOpen');
    if(old&&!old.dataset.fix8923){
      const fresh=old.cloneNode(true);fresh.dataset.fix8923='1';old.replaceWith(fresh);
    ...
    const close=b.querySelector('#nmLiveClose');if(close&&!close.dataset.fix8923){const f=close.cloneNode(true);f.dataset.fix8923='1';close.replaceWith(f);f.addEventListener('click',()=>b.classList.add('hidden'))}
    return b;
  }
  ...
  function showPendingBanner(payload){
    ...
    let label='Получен файл '+(p.name||'NewMatRos')+'. Нажми «Открыть» — появится проверка дилера, материала, ширины и цены перед продажей.';
  ...
  if(window.newmatrosAPI?.onIni){
    window.newmatrosAPI.onIni(payload=>{
      if(!payload?.text)return;
      disableAutoPost();persistPayload(payload);
      try{const data=parsePayload(payload);if(typeof markNewMatRosNotification==='function')markNewMatRosNotification(data)}catch(_){ }
  ...
  const oldBadge=document.getElementById('runtime8921Badge');if(oldBadge)oldBadge.textContent='исправления 8.9.23 активны';
})();
