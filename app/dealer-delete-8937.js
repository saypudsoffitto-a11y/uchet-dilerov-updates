(()=>{
  ...
  // Compatibility code for old releases only. In 8.9.41+ the newer exact-ID
  // handler is loaded directly from preload and must remain authoritative.
  ...
  const install=()=>{
    ...
    function queueSyncPush(){
      ...
      const run=()=>{
        attempts++;
        try{
          if(typeof syncBusy!=='undefined'&&syncBusy){if(attempts<12)setTimeout(run,300);return;}
