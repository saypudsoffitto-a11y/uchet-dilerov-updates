(()=>{
  ...
  function receiptImage8962(op,d){
    ...
    return '<!doctype html><html lang="ru"><head><meta charset="utf-8"><style>*{box-sizing:border-box}html,body{margin:0;background:#fff}body{font-family:Arial,sans-serif;color:#111;width:820px;padding:18px 20px;font-size:14px}h1{text-align:center;font-size:18px;line-height:1.15;margin:0 0 9px;letter-spacing:.02em}.meta{display:grid;grid-template-columns:1fr 1fr;gap:4px 14px;margin-bottom:10px;font-size:13px}table{width:100%;border-collapse:collapse;margin-top:6px;table-layout:fixed;border:2px solid #111}th,td{border:1px solid #444;padding:6px 7px;text-align:left;vertical-align:top}th{background:#e9edf2;font-size:12px;font-weight:700;text-align:center;border-bottom:2px solid #111}th:nth-child(1){width:40px}th:nth-child(2){width:auto}th:nth-child(3){width:92px}th:nth-child(4){width:105px}th:nth-child(5){width:120px}td:nth-child(1){text-align:center}td:nth-child(3),td:nth-child(4),td:nth-child(5){text-align:right}tbody tr:nth-child(even){background:#f5f7fa}.money8962{white-space:nowrap!important;word-break:keep-all!important;font-variant-numeric:tabular-nums}.total,.debt{white-space:nowrap}.total{text-align:right;font-size:17px;font-weight:700;margin-top:10px;padding-top:8px;border-top:2px solid #111}.debt{text-align:right;margin-top:4px;font-size:13px}.sign{display:flex;justify-content:space-between;margin-top:16px;padding-top:10px;border-top:1px solid #888;font-size:12px}</style></head><body><h1>ТОВАРНАЯ НАКЛАДНАЯ № '+h(op.receiptNo)+' от '+h(op.date)+'</h1><div class="meta"><div><b>Поставщик:</b> ____________________</div><div><b>Покупатель:</b> '+h(d?.name||op.dealer||'')+'</div></div><table><thead><tr><th>№</th><th>Наименование</th><th>Кол-во</th><th>Цена</th><th>Сумма</th></tr></thead><tbody>'+rows+'</tbody></table><div class="total">Итого: '+rubHtml(op.total)+'</div><div class="debt"><b>Остаток долга:</b> '+rubHtml(typeof debtOf==='function'?debtOf(op.dealerId):0)+'</div><div class="sign"><span>Отпустил: ____________________</span><span>Получил: ____________________</span></div></body></html>';
  }
  ...
  document.documentElement.dataset.interfaceVersion='8.9.66';
