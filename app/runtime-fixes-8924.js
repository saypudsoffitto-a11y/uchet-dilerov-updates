(()=>{
  ...
  // 8.9.23 exposed a real missing core function: showNewMatRosPreview() and
  // postNewMatRosOrder() call nmFindDealer(), but the function itself was absent
  // from index.html. Define it in the page global scope before the user opens a
  // pending NewMatRos export.
  if(typeof window.nmFindDealer!=='function'){
    window.nmFindDealer=function(data){
      ...
      const rawPhone=String(
        c['Телефон']||c['ТелефонМобильный']||c['МобильныйТелефон']||
        c['Мобильный']||z['Телефон']||''
      ).trim();
      const phone=digits(rawPhone);
      const names=[
        c['Наименование'],c['ФИО'],c['Компания'],c['Контрагент'],
        z['Контрагент'],z['НаименованиеКонтрагента']
      ...
      let dealer=null;
      if(phone){
        dealer=dealers.find(d=>digits(d.phone)===phone)||null;
      }
  ...
  if(old)old.textContent='исправления 8.9.24 активны';
