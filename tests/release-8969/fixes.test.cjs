...
function runtime(){
  ...
  const ctx={
    console,document,localStorage,KEY:'k',setTimeout:fn=>{fn();return 1},clearTimeout(){},setInterval(){return 1},clearInterval(){},MutationObserver:class{observe(){}},
    ...
    ],ops:[],dealers:[],sync:{enabled:false,url:''}},
...
test('NewMatRos reads the current card price on every export',()=>{const c=runtime();let mat=c.nmBuildItems({width:5,['Заказ']:{'МатериалКаталог':'БЕЛАЯ МАТ-303 BAUF'}})[0];assert.equal(mat.price,180);c.state.products.find(p=>p.id===13).retailPrice=195;mat=c.nmBuildItems({width:5,['Заказ']:{'МатериалКаталог':'БЕЛАЯ МАТ-303 BAUF'}})[0];assert.equal(mat.price,195);});
test('missing BAUF card blocks a wrong generic price',()=>{const c=runtime();c.state.products=c.state.products.filter(p=>p.id!==13);const mat=c.nmBuildItems({width:5,['Заказ']:{'МатериалКаталог':'БЕЛАЯ МАТ-303 BAUF'}})[0];assert.equal(mat.price,0);assert.match(mat.priceSource,/BAUF/);});
test('history, group move, optional group and sync hooks are present',()=>{const js=read('app/release-8969.js');assert.match(js,/historySearch8969/);assert.match(js,/№ чека/);assert.match(js,/row\.appendChild\(cells\[i\]\)/);assert.match(js,/groupId:gid/);assert.match(js,/groupId=Number\(editPgroup\?\.value\)\|\|0/);assert.match(js,/updatedAt:Date\.now\(\)/);assert.match(js,/masterSync8962\?\.push/);});
