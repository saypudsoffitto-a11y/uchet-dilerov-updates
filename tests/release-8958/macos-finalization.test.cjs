...
test('8.9.58 is a native Apple Silicon package with ad-hoc signature',()=>{
  ...
  assert.match(pkg.version,/^8\.9\.(58|59|60)$/);
...
test('renderer reports current 8.9.59 runtime',()=>{
  ...
  assert.match(preload,/\?runtime=8960/);
  assert.match(preload,/dataset\.uchetRuntime='8\.9\.60'/);
  assert.match(productFix,/dataset\.interfaceVersion='8\.9\.60'/);
});
