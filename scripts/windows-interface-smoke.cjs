...
async function main(){
  ...
  try{
    ...
    const whatsappTemplate=await evaluate(`(()=>{
      ...
      const html=window.buildReceiptImage8968({receiptNo:42,date:'21.09.2026',dealerId:1,total:4410,items:[{name:'Светильник',qty:9,price:490,total:4410}]},state.dealers[0]);
      return {
        builder:typeof window.buildReceiptImage8968==='function',
        finalHandler:handler.includes('receiptJpegHtml8968'),
        legacyHandler:handler.includes('receiptImage8962'),
        hasTotal:html.includes('Итого:'),
        hasDebt:html.includes('Остаток долга')
      };
    })()`);
    assert.deepEqual(whatsappTemplate,{builder:true,finalHandler:true,legacyHandler:false,hasTotal:true,hasDebt:false},'8.9.68 WhatsApp JPEG template must stay final and must not contain current debt');
    const moneyLayout=await evaluate(`(async()=>{
      ...
      frame.srcdoc=window.buildReceiptImage8968({receiptNo:42,date:'21.09.2026',dealerId:1,total:4410,items:[{name:'Светильник',qty:9,price:490,total:4410}]},state.dealers[0]);
      document.body.appendChild(frame);await loaded;
      const result=[...frame.contentDocument.querySelectorAll('tbody td:nth-child(4),tbody td:nth-child(5)')].map(el=>{
